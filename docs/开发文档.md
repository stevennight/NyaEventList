...
2. 用户点“下载并安装”：下载与当前安装方式一致的安装包（MSI 装的选 MSI，NSIS 装的选 NSIS，靠注册表里的卸载信息判断），**用 `SHA256SUMS` 校验通过才会启动安装程序**，随后本应用退出，由安装程序覆盖安装。数据库在系统的应用数据目录，更新不会动它。
